Vue.createApp({
  ...
  computed: {
    ...
    Für mich zur Erklärung: 
      ---this.books[0]: Greift auf das erste Buchobjekt im books-Array zu
      ---Object.keys(): Diese Funktion gibt ein Array von Schlüsseln (Eigenschaftsnamen)
       eines Objekts zurück. Hier wird Object.keys(this.books[0]) verwendet, 
       um die Schlüssel des ersten Buchobjekts zu extrahieren.
       Beispiel:
       {
    title: 'Book Title',
    author: 'Author Name',
    ...
    Dann würde Object.keys(this.books[0]) ein Array mit den 
    Schlüsseln dieses Objekts zurückgeben:
    ['title', 'author', 'isbn', 'publisher', 'price', 'numPages']
    Diese Schlüssel würden dann für die Generierung der Tabellenüberschriften 
    verwendet werden, indem in der Vue.js-Vorlage eine Schleife (v-for) 
    über diese Schlüssel läuft, um für jede Eigenschaft des Buchobjekts eine 
    Spaltenüberschrift zu erstellen.
      */
    },
    filteredBooks() {
      return this.books
        .filter((book) => book.publisher === "O'Reilly Media")
        .map((book) => ({
          ...book, // <---fügt zwei neue Eigenschaften hinzu, in diesem Fall
          // das originalPrice & discountedPrice
          originalPrice: book.price,
          discountedPrice: this.calcDiscount(book.price),
        }));
    },
  },
  methods: {
